...
public class YonghuzhuceController {
	...
	public String addYonghuzhuce(HttpServletRequest request,Yonghuzhuce yonghuzhuce,HttpSession session) throws SQLException{
		//设置责任链
		IHandler userNameLengthHandler = new UserNameLengthHandler();
		IHandler userNameSameHandler = new UserNameSameHandler();
		IHandler userPasswordHandler = new UserPasswordHandler();
		userNameLengthHandler.setNext(userNameSameHandler);
		userNameSameHandler.setNext(userPasswordHandler);
		RegisterMessageHelper registerMessageHelper = new RegisterMessageHelper(userNameLengthHandler);
		//用户注册责任链校验不通过
		if(!registerMessageHelper.MessageIsTrue(yonghuzhuce)){
